...
function parseData (raw_data_str) {
    ...
    if (t[p++] != "HarryPotter") {
	alert("Read");
	alert("Are you sure this is a HarryPotter game file?");
    ...
    parse_assert(t[p++], "NUM_INI_WIZARDS_PER_CLAN");
    data.nb_ini_wizards_clan = int(t[p++]);

    parse_assert(t[p++], "NUM_INI_BOOKS");
    data.nb_ini_books = int(t[p++]);

    parse_assert(t[p++], "CLAN_INI_MAGIC_STRENGTH");
    data.ini_magic_strength = int(t[p++]);

    parse_assert(t[p++], "POINTS_FOR_CONVERTING_WIZARD");
    data.pts_bewitch_wizard = int(t[p++]);

    parse_assert(t[p++], "POINTS_PER_OWNED_CELL");
    data.pts_owned_cell = int(t[p++]);

    parse_assert(t[p++], "BOOK_MAGIC_STRENGTH");
    data.book_magic_strength = int(t[p++]);

    parse_assert(t[p++], "ROUNDS_FOR_CONVERTING");
    data.rounds_for_converting = int(t[p++]);

    parse_assert(t[p++], "ROUNDS_ATTACK_RESTING_GHOST");
    data.rounds_attack_resting_ghost = int(t[p++]);

    parse_assert(t[p++], "ROUNDS_SPELL_RESTING_GHOST");
    data.rounds_spell_resting_ghost = int(t[p++]);

    parse_assert(t[p++], "ROUNDS_NO_ATTACK_GHOST");
    data.rounds_no_attack_ghost = int(t[p++]);

    data.nb_units = data.nb_players * data.nb_ini_units_clan + 1;
	...
	data.rounds[round].alive = new Array();
	for (var i = 0; i < data.nb_players; ++i) {
	    data.rounds[round].alive[i] = 0;
	}
	...
	for (var i = 0; i < data.rounds[round].units.length; ++i) {
	    ...
	    data.rounds[round].units[i].next   = int(t[p++]);
	    data.rounds[round].alive[data.rounds[round].units[i].player]++;
	}

	// Books.
	parse_assert(t[p++], "books");
	data.rounds[round].books = new Array();
	data.rounds[round].books.length = int(t[p++]);
	parse_assert(t[p++], "row");
	parse_assert(t[p++], "column");
	for (var i = 0; i < data.rounds[round].books.length; ++i) {
	    data.rounds[round].books[i] = new Object();
	    data.rounds[round].books[i].i    = int(t[p++]);
	    data.rounds[round].books[i].j    = int(t[p++]);
	}

	// Spell ingredients.
	parse_assert(t[p++], "spell_ingredients");
	data.rounds[round].spell_ingredients = new Array();
	data.rounds[round].spell_ingredients.length = int(t[p++]);
	for (var i = 0; i < data.rounds[round].spell_ingredients.length; ++i) {
	    data.rounds[round].spell_ingredients[i] = int(t[p++]);
	}

	//Voldemort
	parse_assert(t[p++], "Voldemort");
	data.rounds[round].voldemort_i = int(t[p++]);
	data.rounds[round].voldemort_j = int(t[p++]);
...
function writeGameState () {
    ...
    for (var i = 0; i < data.nb_players; ++i) {
	str_uni = 0;
	if (data.rounds[actRound].alive[i] != 0)
	    str_uni = int(data.rounds[actRound].strength[i]/data.rounds[actRound].alive[i]);
...
//	    + "<div style='padding:2px; width:200px'>Strength:  " + data.rounds[actRound].strength[i] + "</div>"
	    ...
	    + "<div style='padding:2px; width:200px'>Units:  " + data.rounds[actRound].alive[i] + "</div>"
	    + "<div style='padding:2px; width:200px'>Strength/units:  " + str_uni  + "</div>"
...
function drawGame () {
	...
	if      (u.type == 'w' && u.rounds == 0) drawWizard(i, j);
	else if (u.type == 'w') drawConverting(i,j);
	else if (u.type == 'g') drawGhost(i,j);
    }

    // Draw books
    context.strokeStyle = "#000000";
    context.fillStyle = "#FF0000";
    var books = data.rounds[actRound].books;
    for (var fo in books){
	var f = books[fo];
	var i = f.i;
	var j = f.j;
    ...
    // Draw voldemort
    drawVoldemort(data.rounds[actRound].voldemort_i, data.rounds[actRound].voldemort_j, "#FF0000");
}


function drawVoldemort (i, j, col) {
